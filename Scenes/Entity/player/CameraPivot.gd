...
func _input(event):
	...
	if event is InputEventMouseMotion and mouse_lock:
		...
		rotation_degrees.x = clamp(rotation_degrees.x,-45*1.3,45*1.3)
	
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_WHEEL_DOWN) and mouse_lock and spring_arm.spring_length < 15:
		spring_arm.spring_length += 1
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_WHEEL_UP) and mouse_lock and spring_arm.spring_length > 0:
		spring_arm.spring_length -= 1
