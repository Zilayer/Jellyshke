...
const SPEED = 100
const DAMPING = 0.9
@onready var on_floor_left = $"Armature/Skeleton3D/Physical Bone Bone LLeg/OnFloorLeft" # shapecast on the feet to check if its on floor
@onready var on_floor_right = $"Armature/Skeleton3D/Physical Bone Bone RLeg/OnFloorRight" # shapecast on the feet to check if its on floor
@onready var jump_timer = $JumpTimer # timer to stop excidental double jump
var can_jump = true
...
@onready var animated_skel : Skeleton3D = $AnimatedPleyer/Armature/Skeleton3D
@onready var camera_pivot = $CameraPivot
@onready var animation_tree = $AnimationTree
@onready var physical_bone_body : PhysicalBone3D = $"Armature/Skeleton3D/Physical Bone Bone"
...
@onready var grab_joint_right = $GrabJointRight
@onready var grab_joint_left = $GrabJointLeft
@onready var physical_bone_l_arm_2 = $"Armature/Skeleton3D/Physical Bone LArm_2"
@onready var physical_bone_r_arm_2 = $"Armature/Skeleton3D/Physical Bone RArm_2"
@onready var l_grab_area = $"Armature/Skeleton3D/Physical Bone LArm_2/LGrabArea"
@onready var r_grab_area = $"Armature/Skeleton3D/Physical Bone RArm_2/RGrabArea"
...
func _physics_process(delta):
	...
	if not ragdoll_mode:# if not in ragdoll mode
		...
		if Input.is_action_pressed("move_forward"):
			dir += animated_skel.global_transform.basis.y
			walking = true
		...
		animated_skel.rotation.z = camera_pivot.rotation.y
